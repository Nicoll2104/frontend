...
const in_activar = {
  putActivar: async (id) => {
    try {
      ...
      if (index !== -1) {
        rows.value.splice(index, 1, response.data.entradas);
        notificar("positive", "Entrada activada exitosamente");
      } else {
        notificar("negative", "No se encontró la entrada para activar");
      }
    } catch (error) {
      console.log(error);
    }
  ...
  putInactivar: async (id) => {
    try {
      ...
      if (index !== -1) {
        rows.value.splice(index, 1, response.data.entradas);
        notificar("positive", "Entrada inactivada exitosamente");
      } else {
        notificar("negative", "No se encontró la entrada para inactivar");
      }
    } catch (error) {
      console.log(error);
    }
