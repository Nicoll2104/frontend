...
<!-- <q-btn @click="mostrarDetalles(props.row)" label="Mostrar detalles" color="grey-6" class="text-capitalize text-white ">
    ...
    " />
        ...
        <template v-slot:body="props">
          ...
          <div class="show-p" v-if="props.row.mostrarDetalles">
    <q-tr :props="props">
      <q-td colspan="100%" class="">
        ...
        <div class="text-center " v-else>
          <q-btn @click="props.row.mostrarDetalles = false" label="Ocultar detalles" color="grey-6" class="text-capitalize text-white ">
            <q-icon name="remove" color="white" right />
          </q-btn>
        </div>
      </q-td>
    </q-tr>
...
const modelo = "Detalle de pedido";
...
const columns = ref([
  ...
  },
  {
    ...
    label: "Producto",
    align: "left",
    field: (row) => row.producto_id ? row.producto_id.nombre : '',
  },
  {
    name: "subtotal",
    label: "Subtotal",
...
const obtenerInfo = async (idPedido) => {
  try {
    const detPedido = await useDetPedido.obtenerInfodetPedido(idPedido); // Pasar el idPedido al método de obtención de detalles del pedido
...
const mostrarDetalles = (row) => {
  row.mostrarDetalles = true;
}


const buscarDetallesPedido = async () => {
  if (pedidoSeleccionado.value) {
    await obtenerInfo(pedidoSeleccionado.value);
  }
};
...
const opciones = {
  ...
  editar: (info) => {
    data.value = {
      ...info,
      producto_id: { label: info.producto_id.nombre, value: info.producto_id._id },
    };
...
const enviarInfo = {
  guardar: async () => {
    ...
    try {
      const info = { ...data.value, producto_id: data.value.producto_id.value };
  ...
  editar: async () => {
    ...
    try {
      ...
      rows.value.splice(
        buscarIndexLocal(response.data.detPedido._id),
        1,
        response.data.detPedido
      );
...
const in_activar = {
  putActivar: async (id) => {
    try {
      ...
      const index = buscarIndexLocal(response.data.detPedido._id);
      if (index !== -1) {
        // Actualiza solo el estado de la entrada
        rows.value[index].status = response.data.detPedido.status;
...
function validarCampos() {
  const ProductoValidation = validateProducto(data.value.producto_id);
  ...
  for (const d of arrData) {
    ...
    if (d[0] === "producto_id" && d[1].toString().length < 1) {
